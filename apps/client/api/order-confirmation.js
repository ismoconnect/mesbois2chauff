...
module.exports = async function handler(req, res) {
  ...
  try {
    const { orderId, total, items, customer, newUser, lang } = req.body || {};
    if (!customer?.email) {
    ...
    const subject = `${brand} — ${t.subject} ${shortId ? `#${shortId}` : ''}`.trim();

    const itemLines = Array.isArray(items)
      ? items.map((it) => `- ${it.name || t.article} x${it.quantity || 1} — ${(Number(it.price) || 0).toFixed(2)}€`).join('\n')
      : '';

    const addrParts = [customer.address, customer.postalCode, customer.city, customer.country].filter(Boolean);
    const phone = customer.phone ? `${t.phone}: ${customer.phone}` : '';
    ...
    const text = [
      ...
      '',
      intro,
      shortId ? `${t.order_number}: #${shortId}` : '',
      '',
      itemLines ? `${t.order_details}` : '',
      itemLines,
      '',
      `${t.total_ttc}: ${(Number(total) || 0).toFixed(2)}€`,
      addrParts.length ? '' : '',
      addrParts.length ? `${t.delivery_address}:` : '',
    ...
    const rowsHtml = Array.isArray(items)
      ? items.map((it) => {
        ...
        return `
            <tr>
              <td style="padding:8px 12px;border-bottom:1px solid #eee;">${(it.name || t.article)}</td>
      ...
      <div style="font-family:Arial,Helvetica,sans-serif;color:#111;background:#f6f7f9;padding:24px;">
        <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,0.06);overflow:hidden;">
          <div style="padding:24px 24px 12px;border-bottom:1px solid #f0f0f0;background:#fafbfc;">
            ...
            <h1 style="margin:6px 0 0;font-size:18px;color:#2c5530;">${t.subject} ${shortId ? `#${shortId}` : ''}</h1>
            <p style="margin:6px 0 0;color:#555;font-size:14px;">${intro}</p>
          </div>
          <div style="padding:20px 24px;">
            <h2 style="margin:0 0 12px 0;font-size:16px;color:#2c5530;">${t.summary}</h2>
            <table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;font-size:14px;">
              <thead>
                <tr>
                  <th align="left" style="padding:8px 12px;border-bottom:2px solid #e5e7eb;color:#333;font-weight:600;">${t.article}</th>
                  <th align="center" style="padding:8px 12px;border-bottom:2px solid #e5e7eb;color:#333;font-weight:600;">${t.qty}</th>
                  <th align="right" style="padding:8px 12px;border-bottom:2px solid #e5e7eb;color:#333;font-weight:600;">${t.price}</th>
                </tr>
              </thead>
              <tbody>
                ${rowsHtml}
                <tr>
                  <td colspan="2" style="padding:12px 12px;text-align:right;font-weight:700;color:#2c5530;">${t.total_ttc}</td>
            ...
            ${(addrParts.length || phone) ? `
              <div style="margin-top:16px;padding:12px;border:1px solid #eef2f7;border-radius:8px;background:#fafbfd;">
                <div style="font-size:14px;color:#2c5530;font-weight:700;margin-bottom:6px;">${t.delivery_address}</div>
          ...
          <div style="padding:16px 24px;background:#fafbfc;border-top:1px solid #f0f0f0;font-size:12px;color:#6b7280;">
            <div>${brand} — ${t.customer_service}</div>
          </div>
        </div>
      </div>`;
